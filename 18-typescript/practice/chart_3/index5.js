...
// mo.getData(1111)
//test总结
function classDre(params) {
    return function (target) {
        console.log("类装饰器", target);
    };
}
function propClass(params) {
    return function (target, attr) {
        console.log("属性装饰器");
        target[attr] = params;
    };
}
function methodLog(params) {
    return function (target, attr, desc) {
        console.log("方法装饰器");
    };
}
function methodPropLog(params) {
    return function (target, propsName, desc) {
        console.log("方法属性装饰器");
    };
}
var AllPro = /** @class */ (function () {
    function AllPro(parameters) {
    }
    AllPro.prototype.getData = function (uid) {
    };
    __decorate([
        propClass("test-props")
    ], AllPro.prototype, "testProps");
    __decorate([
        methodLog("test-method"),
        __param(0, methodPropLog("test-method-props"))
    ], AllPro.prototype, "getData");
    AllPro = __decorate([
        classDre("")
    ], AllPro);
    return AllPro;
