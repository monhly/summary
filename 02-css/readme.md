...
- :first-child :选取属于其父元素的首个子元素的指定选择器**`如果第一个不是你要找的元素,将找不到该元素`**
- :first-of-type:选取属于其父元素的首个子元素的指定选择器，但是在查找的时候，只会找满足条件的元素，如果不满足，就会过滤掉其他类型的元素**`（直到找到你想要的第一个子元素为止）`**
- :last-child :选取属于其父元素的最后一个子元素的指定选择器
- :last-of-type:选取属于其父元素的最后一个子元素的指定选择器，但是在查找的时候，只会找满足条件的元素，如果不满足，就会过滤掉其他类型的元素
